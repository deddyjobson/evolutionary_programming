'''
Here, I will try to generate strings through sexual reproduction also along
with mutation and survival.

The survival condition will be harsh i.e. if you aren't amongst the best, you
die.

The offspring will be a blend of the parents which is unlike medelinian
genetics.
...
parser.add_argument('--survival_rate',type=float,default=0.01)
parser.add_argument('--dominance_factor',type=float,default=1)
...
def breed(arr1,arr2):
    return ((arr1 + arr2) / 2).astype(np.int_)

def repopulate(old_gen):
    new_gen = [_ for _ in old_gen]
    probs = np.array(list(map(fitness,old_gen)))
    probs -= np.min(probs) - 1
    counts = (probs // np.min(probs)).astype(np.int_)
    counts = counts ** params.dominance_factor
    parents = []
    for i,j in zip(old_gen,counts):
        parents.extend(i for _ in range(j))
    parents = np.array(parents, dtype=np.int_)
    # print(probs)
    # print(counts)
    # print(parents)
    # exit()
    while True:
        if len(new_gen) >= params.population:
            break
        children = mutate( breed(old_gen,np.random.permutation(old_gen)) )
...
if params.plot:
    ...
    plt.ylabel('Fitness')
    plt.plot(np.arange(len(avg_fits))*params.frequency,avg_fits,label='Average fitness')
    plt.plot(np.arange(len(best_fits))*params.frequency,best_fits,label='Best fitness')
    plt.legend()
    plt.show()
